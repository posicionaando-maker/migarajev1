...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Escuchar navegación por botones
    navBtns.forEach(btn => {
...
function manejarHash(hash) {
    ...
    if (cleanHash === 'home' || cleanHash === '') {
        navBtns.forEach(btn => {
            if (btn.dataset.view === 'home') btn.classList.add('active');
        });
        renderizarHome();
        viewActual = 'home';
    } else if (cleanHash === 'categorias') {
        navBtns.forEach(btn => {
            if (btn.dataset.view === 'categorias') btn.classList.add('active');
        });
        renderizarCategorias();
        viewActual = 'categorias';
    } else if (cleanHash === 'reservas') {
        navBtns.forEach(btn => {
            if (btn.dataset.view === 'carrito') btn.classList.add('active');
        });
        renderizarReservas();
        viewActual = 'reservas';
...
function renderizarHome() {
    ...
    if (disponibles.length === 0) {
        html += `<p style="text-align:center;color:var(--text-secondary);padding:40px 0;">No hay productos disponibles en este momento.</p>`;
    } else {
...
function renderizarCard(producto) {
    ...
    return `
        <div class="card ${estadoClase}" onclick="window.location.hash='#producto/${producto.id}'">
...
function renderizarProducto(id) {
    ...
    const detallesHtml = producto.detalles ? Object.entries(producto.detalles).map(([key, val]) => {
        ...
        const labels = {
            ...
            tamaño_maceta: 'Tamaño de maceta',
            cuidado: 'Cuidado'
        };
    ...
    let html = `
        <div class="fade-in">
            <button class="btn-volver" onclick="window.location.hash='${window.history.state?.categoria ? '#categoria/' + window.history.state.categoria : '#home'}'">← Volver</button>
...
// ===== FILTRAR POR CATEGORÍA (desde home) =====
        ...
        qrContainer.innerHTML = `
            <div class="instrucciones-qr">
                ...
                <p style="margin-top:8px;">
                    📌 <strong>Código de reserva:</strong> ${codigo}<br>
                    📅 <strong>Fecha:</strong> ${reserva.fecha}
                </p>
                <p style="margin-top:8px;">
                    Escanea este QR o preséntalo al recoger el producto.
                </p>
            </div>
        `;
    }
    ...
    try {
        ...
        instrucciones.innerHTML = `
            ...
            <ol style="margin: 8px 0 0 20px; color: var(--text-secondary);">
                ...
                <li>Puedes negociar el precio por <a href="tel:${CONFIG.TELEFONO}" style="color:var(--accent);">teléfono</a> o <a href="https://wa.me/${CONFIG.TELEFONO.replace(/[^0-9]/g, '')}" target="_blank" style="color:var(--accent);">WhatsApp</a>.</li>
            </ol>
            <p style="margin-top:8px;font-size:0.9rem;">
                📍 Recogida en: ${CONFIG.DIRECCION}
            </p>
        `;
        ...
        container.innerHTML = `
            <div style="padding:20px;background:var(--bg-primary);border-radius:var(--radius);text-align:center;">
                ...
                <p style="font-size:0.8rem;color:var(--text-secondary);">Preséntalo al recoger.</p>
            </div>
        `;
    }
}
...
function renderizarReservas() {
            ...
            <div class="fade-in">
                ...
                <div class="reserva-vacio">
                    ...
                    <p style="font-size:0.9rem;color:var(--text-secondary);margin-top:8px;">
                        Explora el catálogo y reserva lo que te interese.
                    </p>
    ...
    let html = `
        <div class="fade-in">
            ...
            <p style="color:var(--text-secondary);margin-bottom:16px;">
                Guarda este código QR o preséntalo al recoger.
            </p>
    `;
    ...
    reservas.forEach((reserva, index) => {
        html += `
            <div class="reserva-item">
                ...
                <div class="precio" style="color:var(--accent);">${reserva.precio} ${reserva.moneda}</div>
...
function guardarReservas() {
    try {
        ...
        console.warn('No se pudo guardar reservas en localStorage');
    }
}
